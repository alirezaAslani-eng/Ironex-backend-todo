...
## ۱۳. نیاز به فیلد وضعیت تسویه برای Order

در کنار `settlementMode`، Backend باید یک فیلد جداگانه برای مشخص کردن وضعیت تسویه هر Order ارسال کند.

`settlementMode` فقط مشخص می‌کند که Order مربوط به معامله ۱۰ درصد است یا خیر؛ اما مشخص نمی‌کند که آیا بدهی آن Order هنوز باقی مانده یا تسویه شده است.

بنابراین نیاز است Backend یک Flag مانند `hasRemainingDebt` ارسال کند تا Frontend بتواند تشخیص دهد:

- آیا Order هنوز بدهی قابل پرداخت دارد.
- آیا Order به‌طور کامل تسویه شده است.
